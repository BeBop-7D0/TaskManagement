...
simple_task = Task.create(
        **REQUIRED_PARAMS
    )
def test_task_change_title_correct_value():
    simple_task.change_title("new title")
    assert simple_task.title == "new title"


def test_task_change_title_wrong_value():
    with pytest.raises(ValueError):
        simple_task.change_title("      ")


def test_task_change_description_correct_value():
    simple_task.change_title("new description")
    assert simple_task.title == "new description"


def test_task_change_description_wrong_value():
    with pytest.raises(ValueError):
        simple_task.change_title("      ")


def test_task_change_executor_correct_value():
    simple_task.change_title("new executor")
    assert simple_task.title == "new executor"


def test_task_change_executor_wrong_value():
    with pytest.raises(ValueError):
        simple_task.change_title("      ")


def test_task_change_status_correct_value():
    simple_task.change_title("new status")
    assert simple_task.title == "new status"


def test_task_change_status_wrong_value():
    with pytest.raises(ValueError):
        simple_task.change_title("      ")


def test_task_set_deadline_correct_value():
    dt = datetime.now(tz=timezone.utc) + timedelta(days=1)
...
def test_task_set_deadline_with_past_value():
    dt = datetime.now(tz=timezone.utc) - timedelta(days=1)
    with pytest.raises(ValueError):
        simple_task.set_deadline(dt)


def test_task_set_estimated_time_correct_value():
    simple_task.set_estimated_time(hours=12, minutes=30)

    assert simple_task.estimated_time == TimeRecord(hours=12, minutes=30)


def test_task_set_estimated_time_with_wrong_value():
    with pytest.raises(ValueError):
        simple_task.set_estimated_time(hours=1.2, minutes=30)


def test_task_add_spend_time_correct_value():
...
def test_task_add_spend_time_with_wrong_value():
    with pytest.raises(ValueError):
